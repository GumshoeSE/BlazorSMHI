...
namespace BlazorSMHI.Client.Features.Dashboard.Machines
{
    public partial class Details : IDisposable
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            Machine = await MCDataService.GetMachineDetails(Guid.Parse(MachineId)) ?? new Machine();
            await DrawChart();
        }
        ...
        {
            if (Machine.MachineData.Count == 0) return;
        ...
        private List<List<ChartData>> GetChartData()
        {
            ...
            var hourlyData = Machine.MachineData
                .GroupBy(d => new
                {
                    d.Time.Date,
                    d.Time.Hour
        ...
        {
            var newData = await MCDataService.GetNewMachineData(id);
            if (newData != null)
            {
                Machine.MachineData = newData.ToList();
                StateHasChanged();
